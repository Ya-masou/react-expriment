...
const Child: VFC<ChildProps> = ({ count, doubleCount, handleClick }) => {
  ...
  return (
    <Card title="Child component">
      ...
      <Box paddingY={3}>
        <HStack>
          <Button colorScheme='teal' variant='solid' onClick={handleClick}>Increment child count</Button>
        </HStack>
      </Box>
    </Card>
  )
  ...
  return (
    <VStack spacing={4}>
      <Card title="Parent component">
        ...
        <Box paddingY={3}>
          <HStack>
            <Button colorScheme='teal' variant='solid' onClick={() => setCount(count + 1)}>Increment parent count</Button>
          </HStack>
        </Box>
      </Card>
      <MemoChild count={childCount} doubleCount={doubledChildCount} handleClick={incrementChildCount} />
    </VStack>
  )
}
